import { getFiles, SearchFiles } from '@/utils/actions/fileActions';
  ...
  return (
    <div className='flex flex-col items-center gap-8 w-full max-w-7xl'>
      <section className='w-full'>
        {types}
        <div className='flex justify-between my-3 h-[10%] font-medium text-sm'>
          ...
          {file.map((f: files)=>{
            ...
            return(
              type===types.toLowerCase().slice(0,-1) && <>
                <FileCard key={f._id} file={f} types={type} extrnsion={extension}/>
              </>
            )
          })}
          </div>:<p>No files</p> }
