...
const SidebarBtn = ({pathname,name}:{pathname: string, name: string}) => {
  ...
  return (
    <>
      {name==='Create' ?"":<Link href={`${pathname}`} className={`${path===pathname?'bg-purple-900':'hover:bg-zinc-800'}  flex justify-center lg:justify-start h-13 sm:h-full gap-2 rounded-xl lg:rounded-3xl items-center px-2 lg:px-6 w-13 sm:w-full transition-all duration-200 ease-in-out`}>
